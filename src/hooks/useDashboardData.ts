...
export const useDashboardData = (
    ...
    const pageSize = PAGINATION.clampPageSize(options.pageSize ?? PAGINATION.DEFAULT_PAGE_SIZE);
    const includeDebrief = options.includeDebrief ?? true;

    const dashboardQuery = useQuery({
        queryKey: dashboardKeys.payload(filter, page, options.provider, search),
    ...
    const debriefQuery = useQuery({
        queryKey: dashboardKeys.debrief(),
