...
export default function BillingPageClient({ subscriptions: initialSubscriptions, isPro }: BillingPageClientProps) {
    const [subs, setSubs] = useState<BillingSubscription[]>(initialSubscriptions);
    ...
    const handleGetVirtualCard = async (subscriptionId: string) => {
        ...
        try {
            ...
            setSubs(prev => prev.map(s => s.id === subscriptionId ? { ...s, cardStatus: 'active' } : s));
        } catch (err: any) {
    ...
    const handleToggleFreeze = async (subscriptionId: string, isFrozen: boolean) => {
        setActionLoading(subscriptionId);
        ...
        try {
            ...
            setSubs(prev => prev.map(s => s.id === subscriptionId ? { ...s, cardStatus: data.status } : s));
        } catch {
    ...
    const protectedCount = subs.filter(s => s.cardStatus && s.cardStatus !== 'closed').length;
    const unprotectedCount = subs.filter(s => !s.cardStatus || s.cardStatus === 'closed').length;
    ...
    return (
        <div className="max-w-[960px] mx-auto pt-6 px-6 max-sm:px-4 space-y-4">
            <header className="mb-5">
                <h1 className="text-[28px] font-semibold leading-tight text-[var(--color-text-primary)] font-display">Billing & Cards</h1>
                <p className="text-sm text-[var(--color-text-secondary)] mt-1 font-ui">Manage billing and virtual cards.</p>
            </header>
                ...
                {isPro ? (
                    <div className="grid grid-cols-1 sm:grid-cols-[1.4fr_1fr] gap-4">
                        ...
                        <Card className="p-5 bg-[var(--color-brand-light)] border-[var(--color-brand)]/20 flex flex-col justify-between">
                            <div className="space-y-3">
                                <div>
                                    <Badge variant="default" className="bg-[var(--color-brand)] text-white hover:bg-[var(--color-brand)]">Pro active</Badge>
                                </div>
                                <div>
                                    <h2 className="text-sm font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Current plan</h2>
                                    <p className="text-[13px] text-[var(--color-text-secondary)] mt-1">You have access to virtual cards</p>
                                </div>
                            </div>
                            <div className="mt-6 flex flex-col">
                                <span className="text-[10px] uppercase tracking-widest text-[var(--color-text-secondary)]">Monthly spend</span>
                        ...
                        <Card className="p-5 flex flex-col justify-between">
                            <div className="space-y-3">
                                <div className="flex gap-4">
                                    <div>
                                        <div className="text-[20px] font-semibold text-[var(--color-brand)]">{protectedCount}</div>
                                        <div className="text-[12px] text-[var(--color-text-secondary)]">protected</div>
                                    </div>
                                    <div>
                                        <div className="text-[20px] font-semibold text-[var(--color-text-secondary)]">{unprotectedCount}</div>
                                        <div className="text-[12px] text-[var(--color-text-secondary)]">unprotected</div>
                                    </div>
                                </div>
                            ...
                            <div className="mt-4">
                                <Link href="/dashboard/subscriptions" className="text-xs font-semibold text-[var(--color-brand)] hover:underline inline-flex items-center gap-1">
                    ...
                    <Card className="p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                        <div className="space-y-2">
                            <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Upgrade to Pro</h2>
                        ...
                        <Button variant="primary" onClick={handleUpgrade} disabled={isUpgrading} className="shrink-0">
            ...
            <Card className="p-5 font-ui">
                <div className="flex justify-between items-start gap-4 mb-3.5">
                    <div>
                        <h2 className="text-base font-semibold text-[var(--color-text-primary)]">Subscriptions ready for cards</h2>
                        <p className="text-xs text-[var(--color-text-secondary)]">Issue a dedicated card or review the current setup for each service.</p>
                    </div>
                </div>
                ...
                {subs.length === 0 ? (
                    ...
                    <div className="divide-y divide-[var(--color-border)]/30">
                        {subs.map((sub) => {
                            ...
                            return (
                                <div key={sub.id} className="border-b border-[var(--color-border)]/20 last:border-b-0">
                                    {/* Desktop View */}
                                    <div 
                                    ...
                                    >
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[12px] text-[var(--color-text-secondary)]">monthly</span>
                                        </div>
                                        <div className="text-sm font-medium text-right tabular-nums text-[var(--color-text-primary)]">
                                            {sub.currency === 'USD' ? '$' : '₦'}{sub.amountMonthly.toFixed(2)}
                                        </div>
                                        <div>
                                            {sub.cardStatus && sub.cardStatus !== 'closed' ? (
                                                <Badge variant={isFrozen ? 'secondary' : 'success'}>
                                        ...
                                        <div className="text-right">
                                            {sub.cardStatus && sub.cardStatus !== 'closed' ? (
                                                <Button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        handleToggleFreeze(sub.id, isFrozen);
                                                    }}
                                                ...
                                                >
                                                    {actionLoading === sub.id ? (isFrozen ? '...' : '...') : isFrozen ? 'Unfreeze' : 'Freeze'}
                                                </Button>
                                            ) : (
                                                <Button
                                                    size="sm"
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        handleGetVirtualCard(sub.id);
                                                    }}
                                                    className="w-full text-xs h-[30px]"
                                                >
                                                    {actionLoading === sub.id ? '...' : 'Get card'}
                                                </Button>
                                            )}
                                        </div>
                                    </div>
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <div>
                                                {sub.cardStatus && sub.cardStatus !== 'closed' ? (
                                                    ...
                                                    >
                                                        {actionLoading === sub.id ? '...' : isFrozen ? 'Unfreeze' : 'Freeze'}
                                                    </Button>
                                                ) : (
                                                    <Button
                                                        size="sm"
