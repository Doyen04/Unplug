 ...
 *  4. Pro gate — a cancelled Pro user cannot reveal card details anymore
 */
...
export async function GET(
    ...
    // Pro gate: virtual card numbers must not be revealable after a user cancels Pro.
    ...
    try {
        const { token } = await generateSudoCardToken(
            subscription.sudo_card_id,
        );
        ...
        return NextResponse.json(
            { error: "Failed to generate card token" },
            { status: 502 },
        );
    }
}
